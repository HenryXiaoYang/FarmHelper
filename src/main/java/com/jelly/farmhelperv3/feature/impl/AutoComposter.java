...
public class AutoComposter implements IFeature {
    ...
    private final BlockPos initialComposterPos = BlockPos.containing(-11, 72, -27);
    private Entity composter = null;
    private boolean composterChecked = false;

    private BlockPos composterPos() {
        return BlockPos.containing(FarmHelperConfig.composterX, FarmHelperConfig.composterY, FarmHelperConfig.composterZ);
    }
        ...
        switch (travelState) {
            ...
            case GO_TO_COMPOSTER:
                if (isComposterPosSet()) {
                    if (mc.player.position().distanceToSqr(net.minecraft.world.phys.Vec3.atCenterOf(composterPos())) < 3) {
            ...
            case FIND_COMPOSTER:
                ...
                if (composter == null) {
                    if (!FlyPathFinderExecutor.getInstance().isRunning() && !FarmHelperConfig.autoComposterTravelMethod) {
                        FlyPathFinderExecutor.getInstance().setSprinting(false);
                        FlyPathFinderExecutor.getInstance().setDontRotate(false);
                        FlyPathFinderExecutor.getInstance().findPath(new Vec3(initialComposterPos).add(0.5f, 0.5f, 0.5f), true, true);
                    }
                ...
                LogUtils.sendSuccess("[Auto Composter] Found Composter! " + composter.blockPosition().toString());
                setTravelState(TravelState.GO_TO_COMPOSTER);
        ...
        switch (composterState) {
            ...
            case OPEN_COMPOSTER:
                ...
                if (mop != null && mop.getType() == HitResult.Type.ENTITY) {
                    ...
                    if (entity.equals(composter) || entity.distanceTo(composter) < 1) {
                    ...
                    if (mc.player.distanceTo(composter) > 3) {
                        setMainState(MainState.TRAVEL);
            ...
            case CHECK_COMPOSTER:
                ...
                if (invName.contains("Composter")) {
                    ...
                    ItemStack organicMatterItemStack = organicMatterSlot.getItem();
                    ItemStack fuelItemStack = fuelSlot.getItem();
                    if ((organicMatterItemStack == null || organicMatterItemStack.isEmpty()) || (fuelItemStack == null || fuelItemStack.isEmpty())) break;
                    ...
                    for (String line : organicMatterLore) {
                        Matcher matcher = composterResourcePattern.matcher(ChatFormatting.stripFormatting(line).trim());
                        if (matcher.matches()) {
            ...
            case FILL_COMPOSTER:
                ...
                if (invName2.contains("Composter")) {
                    ...
                    if (boughtOrganicMatterSlot != -1) {
                        InventoryUtils.clickSlotWithId(boughtOrganicMatterSlot, InventoryUtils.ClickType.LEFT, InventoryUtils.ClickMode.PICKUP, chest.containerId);
                        delayClock.schedule(FarmHelperConfig.getRandomGUIMacroDelay());
                        break;
                    }
                    if (boughtFuelSlot != -1) {
                        InventoryUtils.clickSlotWithId(boughtFuelSlot, InventoryUtils.ClickType.LEFT, InventoryUtils.ClickMode.PICKUP, chest.containerId);
                        delayClock.schedule(FarmHelperConfig.getRandomGUIMacroDelay());
                    }
    ...
    private Entity getComposter() {
        return java.util.stream.StreamSupport.stream(mc.level.entitiesForRendering().spliterator(), false)
                .filter(entity -> {
                    ...
                    return entity.getName().getString().contains("COMPOSTER");
                })
