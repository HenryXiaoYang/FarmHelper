...
import com.jelly.farmhelperv3.util.OldRotationUtils;
    ...
    public MovRecPlayer() {
        ...
        try {
            if (FarmHelperConfig.enableCustomReactions) {
                File userFolder = net.fabricmc.loader.api.FabricLoader.getInstance().getConfigDir().resolve("farmhelperv3/data/movrec").toFile();
                ...
                if (resourceFiles.isEmpty()) {
                    ...
                    resourceFiles = getResourceFiles("/assets/farmhelperv3/movrec");
...
//        LogUtils.sendSuccess("easeTo: " + (movement.yaw + yawDifference));
    ...
    private void setPlayerMovement(Movement movement) {
        KeyMapping.set(com.jelly.farmhelperv3.util.KeyBindUtils.boundKey(mc.options.keyUp), movement.forward);
        KeyMapping.set(com.jelly.farmhelperv3.util.KeyBindUtils.boundKey(mc.options.keyLeft), movement.left);
        KeyMapping.set(com.jelly.farmhelperv3.util.KeyBindUtils.boundKey(mc.options.keyDown), movement.backwards);
        KeyMapping.set(com.jelly.farmhelperv3.util.KeyBindUtils.boundKey(mc.options.keyRight), movement.right);
        KeyMapping.set(com.jelly.farmhelperv3.util.KeyBindUtils.boundKey(mc.options.keyShift), movement.sneak);
        mc.player.setSprinting(movement.sprint);
        if (mc.player.getAbilities().mayfly && mc.player.getAbilities().flying != movement.fly) {
        ...
        KeyMapping.set(com.jelly.farmhelperv3.util.KeyBindUtils.boundKey(mc.options.keyJump), movement.jump);
        if (movement.attack && currentDelay == 0)
            KeyBindUtils.leftClick();
        KeyMapping.set(com.jelly.farmhelperv3.util.KeyBindUtils.boundKey(mc.options.keyAttack), movement.attack);
    }

    private List<String> getResourceFiles(String path) throws IOException {
        var directory = net.fabricmc.loader.api.FabricLoader.getInstance().getModContainer("farmhelperv3").orElseThrow().findPath(path.substring(1));
        if (directory.isEmpty()) return List.of();
        try (var files = Files.walk(directory.get())) {
            return files.filter(Files::isRegularFile).filter(file -> file.toString().endsWith(".movement"))
                    .map(file -> path + "/" + directory.get().relativize(file).toString().replace("\\", "/")).sorted().toList();
        }
    }
