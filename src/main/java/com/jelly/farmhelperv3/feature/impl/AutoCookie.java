...
public class AutoCookie implements IFeature {
    ...
    private final BlockPos hubWaypoint = BlockPos.containing(-31, 69, -77);
    ...
    public void onTickUpdate(TickEvent.ClientTickEvent event) {
        ...
        switch (mainState) {
            ...
            case GET_COOKIE:
                switch (bazaarState) {
                    ...
                    case GO_BAZAAR:
                        ...
                        if (Math.sqrt(mc.player.position().distanceToSqr(net.minecraft.world.phys.Vec3.atCenterOf(hubWaypoint))) < 1) {
                            KeyBindUtils.stopMovement();
                        ...
                        Rotation rotationNeeded2 = getRotation().getRotation(new Vec3(hubWaypoint.above().above()).add(new Vec3(0.5, 0.5, 0.5)), true);
                        if (AngleUtils.smallestAngleDifference(mc.player.getYRot(), rotationNeeded2.getYaw()) > 4 ||
                                AngleUtils.smallestAngleDifference(mc.player.getXRot(), rotationNeeded2.getPitch()) > 4) {
                            rotation.easeTo(new RotationConfiguration(rotationNeeded2, FarmHelperConfig.getRandomRotationTime(), null));
                            break;
                        }

                        KeyBindUtils.holdThese(mc.options.keyUp, mc.options.keySprint);
                        break;
                    case OPEN_BAZAAR:
                        if (mc.screen != null) {
                            PlayerUtils.closeContainer();
                        ...
                        Optional<Entity> bazaarNpc = java.util.stream.StreamSupport.stream(mc.level.entitiesForRendering().spliterator(), false).filter(entity -> {
                            double distance = Math.sqrt(entity.position().distanceToSqr(net.minecraft.world.phys.Vec3.atCenterOf(mc.player.blockPosition())));
                        ...
                        if (AngleUtils.smallestAngleDifference(mc.player.getYRot(), rotationNeeded3.getYaw()) > 5 ||
                                AngleUtils.smallestAngleDifference(mc.player.getXRot(), rotationNeeded3.getPitch()) > 5) {
                            KeyBindUtils.stopMovement();
                            rotation.easeTo(
                        ...
                        mc.gameMode.interact(mc.player, bazaarNpc.get(), new net.minecraft.world.phys.EntityHitResult(bazaarNpc.get()), net.minecraft.world.InteractionHand.MAIN_HAND);
                    ...
                    case TELEPORT_TO_GARDEN:
                        ...
                        if (dontEnableClock.isScheduled()) {
                            Tasks.schedule(this::stop, 3_000, TimeUnit.MILLISECONDS);
                            return;
            ...
            case SELECT_COOKIE:
                ...
                mc.player.getInventory().setSelectedSlot(cookieSlot);
