...
public class AutoBazaar implements IFeature {
    ...
    public void click(TickEvent.ClientTickEvent event) {
        if (mc.player == null || mc.level == null || !this.enabled) return;
    ...
    private void handleBuyFromBz() {
        switch (this.buyState) {
            case STARTING:
                ...
                break;
            case OPEN_BZ:
                com.jelly.farmhelperv3.util.PlayerUtils.sendChatMessage("/bz " + this.itemToBuy);
            ...
            case CLICK_ON_PRODUCT:
                ...
                if (itemSlot == -1 || itemSlot > mc.player.containerMenu.slots.size() - 37) {
            ...
            case BUY_INSTANTLY_VERIFY:
                ...
                if (this.openedChestGuiNameStartsWith(this.itemToBuy + " ➜ Instant Buy")) {
                    ...
                    for (Slot slot : buySlots) {
                        String lore = String.join(" ", InventoryUtils.getItemLore(slot.getItem()));
                        ...
                        if (matcher.find() && this.buyAmount == Integer.parseInt(matcher.group(1).replace(",", ""))) {
                            this.buyNowButtonSlot = slot.index;
                            this.buyState = BuyState.CLICK_BUY;
                            System.out.println("Clicking button at slot: " + slot.index);
                            return;
                        }
                    ...
                    if (customAmount != null && customAmount.hasItem()) {
            ...
            case OPEN_SIGN:
                ...
                if (signSlot == null || !signSlot.hasItem()) {
                    this.disable("Could not find sign.");
                    return;
                }

                InventoryUtils.clickContainerSlot(signSlot.index, InventoryUtils.ClickType.LEFT, InventoryUtils.ClickMode.PICKUP);
                this.timer.schedule(2000);
            ...
            case CLICK_BUY:
                ...
                if (buyNowButtonSlot == -1) {
                    Slot customAmount = InventoryUtils.getSlotOfItemInContainer("Custom Amount");
                    if (customAmount != null && customAmount.hasItem()) {
                        this.buyNowButtonSlot = customAmount.index;
                    } else {
                ...
                String lore = String.join(" ", InventoryUtils.getItemLore(slot.getItem())).replace(",", "");
                ...
                InventoryUtils.clickContainerSlot(slot.index, InventoryUtils.ClickType.LEFT, InventoryUtils.ClickMode.PICKUP);
                this.timer.schedule(2000);
                break;
            case BUY_VERIFY:
                if (!InventoryUtils.isInventoryLoaded()) return;
    ...
    private boolean openedChestGuiNameContains(String guiName) {
        ...
        return (mc.screen instanceof ContainerScreen || mc.player.containerMenu instanceof ChestMenu) && (openGuiName != null && openGuiName.contains(guiName));
    }

    private boolean openedChestGuiNameStartsWith(String guiName) {
        String openGuiName = InventoryUtils.getInventoryName();
        System.out.println(openGuiName);
        return (mc.screen instanceof ContainerScreen || mc.player.containerMenu instanceof ChestMenu)
