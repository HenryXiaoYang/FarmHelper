...
import com.jelly.farmhelperv3.util.FailsafeUtils;
...
public class BanInfoWS implements IFeature {
    ...
    public static final File statsDirectory = net.fabricmc.loader.api.FabricLoader.getInstance().getConfigDir().resolve("farmhelperv3/data").toFile();
    ...
    public BanInfoWS() {
        try {
            ...
            if (!Boolean.getBoolean("farmhelperv3.smokeTest")) Tasks.background(() -> {
    ...
    public void onReceivePacket(ReceivePacketEvent event) {
        ...
        if (packet instanceof ClientboundDisconnectPacket) {
            String reason = ((ClientboundDisconnectPacket) packet).reason().getString();
    ...
    private void processBanScreen(String wholeReason) {
        ...
        try {
            ...
            String duration = ChatFormatting.stripFormatting(multilineMessage.get(0)).replace("You are temporarily banned for ", "")
                    .replace(" from this server!", "").trim();
            String reason = ChatFormatting.stripFormatting(multilineMessage.get(2)).replace("Reason: ", "").trim();
            int durationDays = Integer.parseInt(duration.split(" ")[0].replace("d", ""));
            String banId = ChatFormatting.stripFormatting(multilineMessage.get(5)).replace("Ban ID: ", "").trim();
    ...
    public void playerBanned(int days, String reason, String banId, String fullReason) {
        ...
        jsonObject.addProperty("uuid", Minecraft.getInstance().getUser().getProfileId().toString());
        jsonObject.addProperty("mod", "farmHelper");
        jsonObject.addProperty("username", Minecraft.getInstance().getUser().getName());
    ...
    private void collectMods(JsonObject obj) {
        ...
        for (ModContainer mod : FabricLoader.getInstance().getAllMods()) {
            mods.addProperty(mod.getMetadata().getName(), mod.getMetadata().getId());
        ...
        for (File mod : Objects.requireNonNull(new File(Minecraft.getInstance().gameDirectory, "mods").listFiles())) {
        ...
        if (ctLoaded) {
            for (File module : Objects.requireNonNull(new File(Minecraft.getInstance().gameDirectory, "config/ChatTriggers/modules").listFiles())) {
        ...
        try {
            ...
            File oneConfigFolder = new File(Minecraft.getInstance().gameDirectory, "OneConfig/profiles/Default Profile");
            if (!oneConfigFolder.exists()) {
                oneConfigFolder = new File(Minecraft.getInstance().gameDirectory, "OneConfig/config");
            }

            File oringoFile = new File(Minecraft.getInstance().gameDirectory, "config/OringoClient/OringoClient.json");
            ...
            File skyskippedFile = new File(Minecraft.getInstance().gameDirectory, "config/skyskipped/config.json");
            if (ssLoaded && skyskippedFile.exists()) {
                configFiles.addProperty("skyskipped", compress(skyskippedFile));
            }

            File skyskippedLoaderFile = new File(Minecraft.getInstance().gameDirectory, "config/skyskippedloader/config.json");
            ...
            File pizzaFile = new File(Minecraft.getInstance().gameDirectory, "config/pizzaclient/config.json");
            ...
            File auroraFile = new File(Minecraft.getInstance().gameDirectory, "config/aurora.toml");
            if (auroraLoaded && auroraFile.exists()) {
                configFiles.addProperty("aurora", compress(auroraFile));
            }

            File cheetoFile = new File(Minecraft.getInstance().gameDirectory, "Cheeto/configs/Client.json");
    ...
    public void saveStats() {
        JsonObject jsonObject = new JsonObject();
        jsonObject.addProperty("id", Minecraft.getInstance().getUser().getProfileId().toString());
    ...
    private JsonObject getHeaders() {
        ...
        handshake.addProperty("id", Minecraft.getInstance().getUser().getProfileId().toString()); // that's public uuid bozos, not a token to login
        handshake.addProperty("username", Minecraft.getInstance().getUser().getName());
    ...
    private String mojangAuthentication() throws AuthenticationException {
        ...
        try {
            Minecraft.getInstance().services().sessionService().joinServer(Minecraft.getInstance().getUser().getProfileId(), Minecraft.getInstance().getUser().getAccessToken(), serverId);
        } catch (AuthenticationException e) {
