...
public class VisitorsMacro implements IFeature {
    ...
    @Getter
    private final ArrayList<Pair<String, String>> currentRewards = new ArrayList<>();
    ...
    public boolean canEnableMacro(boolean manual, boolean withError) {
        ...
        List<String> visitorsWithoutIgnored = visitors.stream().filter(s -> ignoredNPCs.stream().noneMatch(s2 -> ChatFormatting.stripFormatting(s2.getName().getString()).contains(ChatFormatting.stripFormatting(s)))).collect(Collectors.toList());
        List<String> visitorsWithoutServed = visitorsWithoutIgnored.stream().filter(s -> servedCustomers.stream().noneMatch(s2 -> ChatFormatting.stripFormatting(s2.getName().getString()).contains(ChatFormatting.stripFormatting(s)))).collect(Collectors.toList());
    ...
    public void onTickCheckVisitors(TickEvent.ClientTickEvent event) {
        ...
        for (String line : tabList) {
            ...
            if (ChatFormatting.stripFormatting(line).trim().isEmpty()
                    || newVisitors.size() == 5
                    || ChatFormatting.stripFormatting(line).trim().contains("Next Visitor")) {
                if (foundVisitors) {
                    break;
                }
                continue;
    ...
    private void onTravelState() {
        ...
        switch (travelState) {
            case NONE:
                ...
                break;
            case WAIT_FOR_TP:
                if (mc.player.position().distanceToSqr(net.minecraft.world.phys.Vec3.atCenterOf(positionBeforeTp)) < 3 || PlayerUtils.isPlayerSuffocating()) {
    ...
    private void onCompactorState() {
        switch (compactorState) {
            ...
            case GET_LIST:
                ...
                if (!foundRotationToEntity) {
                    ...
                    if (GameStateHandler.getInstance().isBackWalkable()) {
                        KeyBindUtils.holdThese(mc.options.keyDown);
                        Tasks.schedule(KeyBindUtils::stopMovement, 150, TimeUnit.MILLISECONDS);
                    } else if (GameStateHandler.getInstance().isLeftWalkable()) {
                        KeyBindUtils.holdThese(mc.options.keyLeft);
                        Tasks.schedule(KeyBindUtils::stopMovement, 150, TimeUnit.MILLISECONDS);
                    } else if (GameStateHandler.getInstance().isRightWalkable()) {
                        KeyBindUtils.holdThese(mc.options.keyRight);
                        Tasks.schedule(KeyBindUtils::stopMovement, 150, TimeUnit.MILLISECONDS);
                    } else {
            ...
            case HOLD_COMPACTOR:
                ...
                mc.player.getInventory().setSelectedSlot(compactors.get(0));
    ...
    private boolean findRotation(Vec3 playerPos, int i) {
        float yawToCheck = (float) (mc.player.getYRot() + (i * 15) + Math.random() * 5 - 2.5);
        float pitchToCheck = mc.player.getXRot() + (float) (Math.random() * 5 - 2.5);
        ...
        List<Entity> entitiesInFront = mc.level.getEntities(mc.player, mc.player.getBoundingBox().expandTowards(testRotation.x * 5, testRotation.y * 5, testRotation.z * 5).inflate(1, 1, 1));
        if (!entitiesInFront.isEmpty()) {
            for (Entity entity : entitiesInFront) {
                AABB entityBoundingBox = entity.getBoundingBox().inflate(entity.getPickRadius(), entity.getPickRadius(), entity.getPickRadius());
                HitResult movingObjectPosition = net.minecraft.world.phys.shapes.Shapes.create(entityBoundingBox).clip(playerPos, lookVector, BlockPos.ZERO);

                if (entityBoundingBox.contains(playerPos)) {
    ...
    private void onVisitorsState() {
        switch (visitorsState) {
            ...
            case SELECT_NEW_VISITOR:
                ...
                LogUtils.sendDebug("Position of visitor: " + result.entityCharacter.getEyePosition(1));
            ...
            case GET_CLOSE_TO_VISITOR:
                ...
                if (PlayerUtils.getFarmingTool(MacroHandler.getInstance().getCrop(), true, true) != -1) {
                    mc.player.getInventory().setSelectedSlot(PlayerUtils.getFarmingTool(MacroHandler.getInstance().getCrop(), true, true));
                }

                assert currentCharacter.isPresent();
                if (mc.player.distanceTo(currentCharacter.get()) > 3) {
                    ...
                    if (FarmHelperConfig.visitorsExchangeTravelMethod)
                        BaritoneHandler.walkCloserToBlockPos(currentCharacter.get().blockPosition(), 2);
                    else {
                        FlyPathFinderExecutor.getInstance().setSprinting(false);
                        FlyPathFinderExecutor.getInstance().setDontRotate(true);
                        FlyPathFinderExecutor.getInstance().findPath(closestVec.add(0, 1.8, 0), false, true);
                    }
                }
                RotationHandler.getInstance().easeTo(
                        new RotationConfiguration(
            ...
            case OPEN_VISITOR:
                ...
                if (currentCharacter.isPresent() && mc.player.distanceTo(currentCharacter.get()) > 3) {
                ...
                LogUtils.sendDebug("[Visitors Macro] Distance: " + mc.player.distanceTo(currentCharacter.get()));
                setVisitorsState(VisitorsState.GET_CLOSE_TO_VISITOR);
                break;
            case CHECK_VISITOR:
                if (mc.screen == null) {
                ...
                ItemStack npcItemStack = npcSlot.getItem();
                if ((npcItemStack == null || npcItemStack.isEmpty())) break;
                ...
                ItemStack acceptOfferItemStack = acceptOfferSlot.getItem();
                if ((acceptOfferItemStack == null || acceptOfferItemStack.isEmpty())) {
                ...
                for (String line : loreAcceptOffer) {
                    ...
                    if (foundRequiredItems) {
                        Matcher matcher = itemNamePattern.matcher(ChatFormatting.stripFormatting(line).trim());
                        if (matcher.matches()) {
                    ...
                    if (foundRewards) {
                        System.out.println(ChatFormatting.stripFormatting(line));
                        if (ChatFormatting.stripFormatting(line).trim().isEmpty()) {
                            foundRewards = false;
                            continue;
                        }
                        if (line.contains("+")) {
                            String[] split = ChatFormatting.stripFormatting(line).trim().split(" ");
                            String amount = split[0].trim();
                            String item = String.join(" ", Arrays.copyOfRange(split, 1, split.length)).trim();
                            currentRewards.add(Pair.of(item, amount));
                        } else {
                            int amount = 1;
                            String item = ChatFormatting.stripFormatting(line).trim();
                            currentRewards.add(Pair.of(item, String.valueOf(amount)));
                        }
                    }
                }

                LogUtils.sendDebug("Current Rewards:");
                for (Pair<String, String> currentReward : currentRewards) {
                    LogUtils.sendDebug(currentReward.getLeft() + " " + currentReward.getRight());
                }
                ...
                Optional<Pair<String, String>> profitableReward = currentRewards.stream().filter(item -> {
                    String name = ChatFormatting.stripFormatting(item.getLeft());
                ...
                if (profitNpc && FarmHelperConfig.sendVisitorsMacroLogs) {
                    String reward = ChatFormatting.stripFormatting(profitableReward.get().getLeft());
                ...
                if (FarmHelperConfig.filterVisitorsByName) {
                    if (FarmHelperConfig.nameFilter.isEmpty()) {
                        ...
                        if (visitorsList.stream().anyMatch(visitorName -> ChatFormatting.stripFormatting(npcName.toLowerCase()).contains(visitorName.toLowerCase()))) {
            ...
            case FINISH_VISITOR:
                ...
                String inventoryName = ChatFormatting.stripFormatting(InventoryUtils.getInventoryName());
                ...
                if (currentVisitor.isPresent() && !currentVisitor.get().getName().getString().contains(inventoryName)) {
                    PlayerUtils.closeContainer();
                ...
                ItemStack acceptOfferItemStack2 = acceptOfferSlot2.getItem();
                if ((acceptOfferItemStack2 == null || acceptOfferItemStack2.isEmpty())) {
                    LogUtils.sendError("[Visitors Macro] Couldn't find the \"Accept Offer\" slot!");
                    delayClock.schedule(getRandomDelay());
                    break;
                }
                ...
                haveItemsInSack = false;
                InventoryUtils.clickContainerSlot(acceptOfferSlot2.index, InventoryUtils.ClickType.LEFT, InventoryUtils.ClickMode.PICKUP);
                if (FarmHelperConfig.sendVisitorsMacroLogs) {
                    assert currentVisitor.isPresent();
                    LogUtils.webhookLog("[Visitors Macro]\\nVisitors Macro accepted visitor: " + ChatFormatting.stripFormatting(currentVisitor.get().getName().getString()), FarmHelperConfig.pingEveryoneOnVisitorsMacroLogs, currentRewards.toArray(new Pair[0]));
                }
    ...
    private VisitorEntities getVisitorEntities(Entity entity) {
        ...
        if (entity instanceof ArmorStand) {
            if (entity.getName().getString().contains("CLICK")) {
                Entity realName = PlayerUtils.getEntityCuttingOtherEntity(entity, (entity2 -> (entity2 instanceof ArmorStand) && !entity2.equals(entity)));
                if (realName == null) {
                ...
                Entity realClick = PlayerUtils.getEntityCuttingOtherEntity(entity, (entity2 -> (entity2 instanceof ArmorStand) && entity2.getName().getString().contains("CLICK")));
                if (realClick == null) {
            ...
            Entity realCharacter = PlayerUtils.getEntityCuttingOtherEntity(nameArmorStand, (entity2 -> !(entity2 instanceof ArmorStand) && !entity2.equals(mc.player)));
            ...
            Entity realName = PlayerUtils.getEntityCuttingOtherEntity(entity, (entity2 -> (entity2 instanceof ArmorStand) && !entity2.getName().getString().contains("CLICK")));
            if (realName == null) {
                LogUtils.sendError("[Visitors Macro] Couldn't find the real name of the visitor");
                return null;
            }
    ...
    private boolean equalsWithoutFormatting(String name1, String name2) {
        return ChatFormatting.stripFormatting(name1.toLowerCase()).trim().equals(ChatFormatting.stripFormatting(name2.toLowerCase()).trim());
    }

    private boolean moveAwayIfPlayerTooClose() {
        try {
            assert currentCharacter.isPresent();
            LogUtils.sendDebug("Distance: " + mc.player.distanceTo(currentCharacter.get()));
            if (mc.player.distanceTo(currentCharacter.get()) < 0.25) {
                FlyPathFinderExecutor.getInstance().setSprinting(false);
                FlyPathFinderExecutor.getInstance().setDontRotate(true);
                ...
                if (closest == null) {
                    ...
                    FlyPathFinderExecutor.getInstance().findPath(this.closestVec.get().add(0, 1.8, 0), false, true);
                    return true;
                }
                if (GameStateHandler.getInstance().isBackWalkable()) {
                    LogUtils.sendDebug("[Visitors Macro] Too close to character. Moving back!");
                    KeyBindUtils.holdThese(mc.options.keyDown, !mc.player.getAbilities().flying && GameStateHandler.getInstance().getSpeed() > 250 && (mc.player.getDeltaMovement().x > 0.1 || mc.player.getDeltaMovement().z > 0.1) ? mc.options.keyShift : null);
                    Tasks.schedule(KeyBindUtils::stopMovement, 50, TimeUnit.MILLISECONDS);
                    return true;
                }
    ...
    private boolean moveSideways() {
        if (GameStateHandler.getInstance().isLeftWalkable()) {
            KeyBindUtils.holdThese(mc.options.keyLeft, !mc.player.getAbilities().flying && GameStateHandler.getInstance().getSpeed() > 250 && (mc.player.getDeltaMovement().x > 0.1 || mc.player.getDeltaMovement().z > 0.1) ? mc.options.keyShift : null);
            Tasks.schedule(KeyBindUtils::stopMovement, 50, TimeUnit.MILLISECONDS);
            return true;
        }
        if (GameStateHandler.getInstance().isRightWalkable()) {
            KeyBindUtils.holdThese(mc.options.keyRight, !mc.player.getAbilities().flying && GameStateHandler.getInstance().getSpeed() > 250 && (mc.player.getDeltaMovement().x > 0.1 || mc.player.getDeltaMovement().z > 0.1) ? mc.options.keyShift : null);
            Tasks.schedule(KeyBindUtils::stopMovement, 50, TimeUnit.MILLISECONDS);
            return true;
        }
        return false;
    ...
    private void rejectCurrentVisitor() {
        ...
        InventoryUtils.clickContainerSlot(rejectOfferSlot.index, InventoryUtils.ClickType.LEFT, InventoryUtils.ClickMode.PICKUP);
        assert currentVisitor.isPresent();
        if (FarmHelperConfig.sendVisitorsMacroLogs)
            LogUtils.webhookLog("[Visitors Macro]\\nVisitors Macro rejected visitor: " + ChatFormatting.stripFormatting(currentVisitor.get().getName().getString()), FarmHelperConfig.pingEveryoneOnVisitorsMacroLogs, currentRewards.toArray(new Pair[0]));
        currentVisitor.ifPresent(servedCustomers::add);
        delayClock.schedule(FarmHelperConfig.getRandomGUIMacroDelay());
        setVisitorsState(VisitorsState.SELECT_NEW_VISITOR);
    }
