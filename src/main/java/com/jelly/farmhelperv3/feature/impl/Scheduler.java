...
public class Scheduler implements IFeature {
  ...
  public String getStatusString() {
    if (FarmHelperConfig.enableScheduler) {
      return (schedulerState == SchedulerState.FARMING ? (ChatFormatting.GREEN + "Farming") : (ChatFormatting.DARK_AQUA + "Break"))
          + ChatFormatting.RESET + " for " +
          ChatFormatting.GOLD + LogUtils.formatTime(Math.max(schedulerClock.getRemainingTime(), 0)) + ChatFormatting.RESET + (
    ...
    if (FarmHelperConfig.schedulerDisconnectDuringBreak) {
      Tasks.schedule(() -> {
        mc.getConnection().getConnection()
            .disconnect(Component.literal("Will reconnect after break ends! Restart if you don't see a break timer."));
      ...
      Tasks.schedule(() -> {
        ...
        this.rotation.easeTo(
            new RotationConfiguration(
                new Rotation(
                    (float) (mc.player.getYRot() + Math.random() * 20 - 10),
                    (float) Math.min(90, Math.max(-90, (mc.player.getXRot() + Math.random() * 30 - 15)))
                ),
                randomTime4, null
            )
        );
        if (FarmHelperConfig.openInventoryOnSchedulerBreaks) {
          Tasks.schedule(InventoryUtils::openInventory, randomTime4 + 350, TimeUnit.MILLISECONDS);
        }
  ...
  public void onKeyPress(GuiScreenEvent.KeyboardInputEvent event) {
    ...
    if (FarmHelperConfig.toggleMacro.getKeyBinds().contains(Input.getEventKey())) {
      farmingTime();
    }
  }
