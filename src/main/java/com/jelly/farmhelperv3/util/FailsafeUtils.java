...
import com.jelly.farmhelperv3.util.helper.KeyCodeConverter;
...
public class FailsafeUtils {
    ...
    public FailsafeUtils() {
        ...
        BufferedImage image;
        try {
            image = ImageIO.read(Objects.requireNonNull(getClass().getResource("/assets/farmhelperv3/icon-mod/rat.png")));
        } catch (IOException e) {
    ...
    public static void bringWindowToFrontUsingWinApi() {
        try {
            ...
            WinDef.HWND hWnd = new WinDef.HWND(com.sun.jna.Pointer.createConstant(org.lwjgl.glfw.GLFWNativeWin32.glfwGetWin32Window(Minecraft.getInstance().getWindow().handle())));
            if (hWnd == null) {
    ...
    public static void bringWindowToFrontUsingRobot() {
        SwingUtilities.invokeLater(() -> {
            int TAB_KEY = (net.minecraft.util.Util.getPlatform() == net.minecraft.util.Util.OS.OSX) ? KeyEvent.VK_META : KeyEvent.VK_ALT;
            try {
                Robot robot = new Robot();
                int i = 0;
                while (!Minecraft.getInstance().isWindowActive()) {
                    i++;
