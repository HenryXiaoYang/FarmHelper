...
public class PlotUtils {
    private static final File plotsFile = net.fabricmc.loader.api.FabricLoader.getInstance().getConfigDir().resolve("farmhelperv3/plots.json").toFile();
    @Getter
    ...
    public static List<Pair<Integer, Integer>> getPlotChunksBasedOnLocation(BlockPos pos) {
        LevelChunk chunk = mc.level.getChunkAt(pos);
        for (Map.Entry<Integer, Plot> entry : PLOTS.entrySet()) {
            if (entry.getValue().chunks.stream().anyMatch(t -> t.getLeft() == chunk.getPos().x() && t.getRight() == chunk.getPos().z())) {
    ...
    public static List<Pair<Integer, Integer>> getPlotChunksBasedOnLocation() {
        AABB playerBoundingBox = mc.player.getBoundingBox();
        // get blockpos in the middle of the player's bounding box
        BlockPos pos = BlockPos.containing(playerBoundingBox.minX + (playerBoundingBox.maxX - playerBoundingBox.minX) / 2, playerBoundingBox.minY, playerBoundingBox.minZ + (playerBoundingBox.maxZ - playerBoundingBox.minZ) / 2);
    ...
    public static List<Pair<Integer, Integer>> getPlotChunksBasedOnNumber(int plotNumber) {
        return PLOTS.get(plotNumber).chunks;
    }

    private static List<Pair<Integer, Integer>> getChunks(int fromX, int toX, int fromZ, int toZ) {
    ...
    public static BlockPos getPlotCenter(int plotNumber) {
        List<Pair<Integer, Integer>> chunks = getPlotChunksBasedOnNumber(plotNumber);
        int minX = chunks.get(0).getLeft() * 16;
        int maxX = chunks.get(chunks.size() - 1).getLeft() * 16;
        int minZ = chunks.get(0).getRight() * 16;
        int maxZ = chunks.get(chunks.size() - 1).getRight() * 16;
        float centerX = (float) (minX + (maxX - minX) / 2);
        float centerZ = (float) (minZ + (maxZ - minZ) / 2);
        return BlockPos.containing(centerX, 80, centerZ);
    }

    public static class Plot {
        public Integer number;
        public String name;
        List<Pair<Integer, Integer>> chunks;
        ...
        public Plot addAll(List<Pair<Integer, Integer>> chunks) {
