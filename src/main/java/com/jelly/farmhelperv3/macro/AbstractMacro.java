...
import com.jelly.farmhelperv3.failsafe.impl.RotationFailsafe;
import com.jelly.farmhelperv3.feature.impl.DesyncChecker;
...
public abstract class AbstractMacro {
    ...
    public void click() {
        if (FailsafeManager.getInstance().triggeredFailsafe.isPresent() || FailsafeManager.getInstance().getChooseEmergencyDelay().isScheduled()) {
            if (!sentWarning) {
                LogUtils.sendWarning("Failsafe is running! Blocking main click event!");
                sentWarning = true;
            }
            return;
        }
        ...
        if (mc.player.blockPosition().getY() < 0) {
        ...
        if (rewarpState == RewarpState.TELEPORTED) {
            // rotate
            if (rotated && !LagDetector.getInstance().isLagging() && mc.player.onGround()) {
                LogUtils.sendDebug("Rotated after warping.");
                rewarpState = RewarpState.POST_REWARP;
                Tasks.schedule(() -> rewarpState = RewarpState.NONE, 600, TimeUnit.MILLISECONDS);
                rewarpDelay.schedule(FarmHelperConfig.getRandomTimeBetweenChangingRows());
                return;
            } else if (rotated && (LagDetector.getInstance().isLagging() || !mc.player.onGround())) {
            ...
            if (shouldRotateAfterWarp()) {
                ...
                Rotation curretnRotation = new Rotation(mc.player.getYRot(), mc.player.getXRot());
        ...
        if (GameStateHandler.getInstance().canChangeDirection()) {
            ...
            if (getCurrentState().equals(State.DROPPING) && !mc.player.onGround()) {
                return;
            }
            updateState();
            ...
            if (!mc.player.onGround() && Math.abs(layerY - mc.player.getY()) > 0.75 && mc.player.getY() < 80 && !getCurrentState().equals(State.DROPPING)) {
    ...
    protected void setWalkingDirection() {
        ...
        setPreviousWalkingCoord(getWalkingDirection() == WalkingDirection.X ? mc.player.blockPosition().getZ() : currentX);
    }
