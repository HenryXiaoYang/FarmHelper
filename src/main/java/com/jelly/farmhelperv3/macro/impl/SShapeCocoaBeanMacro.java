...
public class SShapeCocoaBeanMacro extends AbstractMacro {
        ...
        switch (getCurrentState()) {
            ...
            case FORWARD:
                KeyBindUtils.holdThese(
                        mc.options.keyUp,
                        mc.options.keyAttack,
                        shouldHugWall() ? mc.options.keyLeft : null
                );
                break;
            case SWITCHING_LANE:
                if (hasLineChanged()
    ...
    private boolean hasLineChanged() {
        if (BlockUtils.getRelativeBlock(-1, 0, 1).defaultBlockState().isSolid() && GameStateHandler.getInstance().isFrontWalkable()) {
            double decimalPartX = Math.abs(mc.player.position().x) % 1;
            double decimalPartZ = Math.abs(mc.player.position().z) % 1;
            float yaw = AngleUtils.getClosest(mc.player.getYRot());
            yaw = (yaw % 360 + 360) % 360;
    ...
    private boolean shouldHugWall() {
        if (BlockUtils.getRelativeBlock(-1, 0, 0).defaultBlockState().isSolid() && !BlockUtils.getRelativeBlock(-1, 0, 0).equals(Blocks.AIR)) {
            double x = mc.player.position().x % 1;
            double z = mc.player.position().z % 1;
            float yaw = AngleUtils.getClosest(mc.player.getYRot());
            yaw = (yaw % 360 + 360) % 360;
