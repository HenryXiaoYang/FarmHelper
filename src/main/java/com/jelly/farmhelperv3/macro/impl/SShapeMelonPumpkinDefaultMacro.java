...
import static com.jelly.farmhelperv3.util.BlockUtils.getRelativeBlock;
import static com.jelly.farmhelperv3.util.BlockUtils.getRelativeBlockPos;
        ...
        switch (getCurrentState()) {
            case RIGHT:
                KeyBindUtils.holdThese(
                        isHuggingBackWall() ? mc.options.keyUp : null,
                        mc.options.keyRight,
            ...
            case SWITCHING_LANE:
                double velocity = Math.abs(mc.player.getDeltaMovement().x) + Math.abs(mc.player.getDeltaMovement().z);
