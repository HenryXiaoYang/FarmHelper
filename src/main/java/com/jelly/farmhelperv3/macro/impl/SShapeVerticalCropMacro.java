        ...
        switch (getCurrentState()) {
            ...
            case SWITCHING_LANE: {
                if (getWalkingDirection() == WalkingDirection.X) {
                    int currentZ = mc.player.blockPosition().getZ();
                    if (lagBackDetected(currentZ)) break;
                } else if (getWalkingDirection() == WalkingDirection.Z) {
                    int currentX = mc.player.blockPosition().getX();
            ...
            case DROPPING: {
                LogUtils.sendDebug("onGround: " + mc.player.onGround());
        ...
        switch (getCurrentState()) {
            case LEFT:
                KeyBindUtils.holdThese(
                        ...
                        PlayerUtils.shouldWalkForwards() || FarmHelperConfig.alwaysHoldW ? mc.options.keyUp : null
                );
                break;
            case RIGHT:
                KeyBindUtils.holdThese(
            ...
            case SWITCHING_LANE:
                ...
                switch (changeLaneDirection) {
                    case FORWARD:
                        KeyBindUtils.holdThese(mc.options.keyUp, mc.options.keySprint, FarmHelperConfig.holdLeftClickWhenChangingRow ? mc.options.keyAttack : null);
                        break;
                    case BACKWARD:
                        KeyBindUtils.holdThese(mc.options.keyDown, FarmHelperConfig.holdLeftClickWhenChangingRow ? mc.options.keyAttack : null);
                        break;
                    default: {
