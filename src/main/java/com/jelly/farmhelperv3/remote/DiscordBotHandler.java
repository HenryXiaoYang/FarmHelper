...
import com.jelly.farmhelperv3.remote.command.discordCommands.DiscordCommand;
import com.jelly.farmhelperv3.remote.command.discordCommands.impl.*;
import com.jelly.farmhelperv3.remote.event.InteractionAutoComplete;
import com.jelly.farmhelperv3.remote.event.InteractionCreate;
...
public class DiscordBotHandler extends ListenerAdapter {
    ...
    public void click(TickEvent.ClientTickEvent e) {
        ...
        if (!FarmHelperConfig.enableRemoteControl) {
            ...
            connectingState = ChatFormatting.YELLOW + "Connecting to Socket...";
            return;
        }
        ...
        if (WebsocketHandler.getInstance().getWebsocketState() == WebsocketHandler.WebsocketState.CLIENT) {
            if (WebsocketHandler.getInstance().getWebsocketClient() != null && WebsocketHandler.getInstance().getWebsocketClient().isOpen()) {
                connectingState = ChatFormatting.GREEN + "Connected to the Discord Bot as a client";
            } else {
                connectingState = ChatFormatting.GOLD + "Connecting to the Discord Bot as a client...";
            }
            return;
        }

        if (jdaClient != null && jdaClient.getStatus() == JDA.Status.CONNECTED) {
            connectingState = ChatFormatting.DARK_GREEN + "Connected to Discord Bot as a server";
        } else {
            connectingState = ChatFormatting.GOLD + "Connecting to the Discord Bot as a server...";
        }
    }
}
