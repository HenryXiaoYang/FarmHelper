...
import com.jelly.farmhelperv3.failsafe.impl.GuestVisitFailsafe;
...
public class DebugHUD extends TextHud {
    public DebugHUD() {
        super(true, 1f, 10f, 0.5f, true, true, 1, 5, 5, new ConfigColor(0, 0, 0, 150), false, 2, new ConfigColor(0, 0, 0, 127));
    }
...
//        }
        ...
        if (PestsDestroyer.getInstance().isRunning()) {
            ...
            PestsDestroyer.getInstance().getCurrentEntityTarget().ifPresent(target -> lines.add(
                    String.format("   Current Entity Target: %.2f %.2f %.2f", target.position().x, target.position().y, target.position().z))
            );
//            lines.add("PD OTT:");
...
//            lines.add("   entities: ");
//            for (Pair<Entity, Double> entity : PestsDestroyerOnTheTrack.getInstance().getEntities()) {
//                lines.add("   " + entity.getLeft().blockPosition() + " Yaw diff: " + String.format("%.2f", entity.getRight()));
//            }
        }
        ...
        if (BPSTracker.getInstance().isRunning()) {
            ...
            if (!BPSTracker.getInstance().bpsQueue.isEmpty()
                ...
                lines.add("   First timestamp: " + BPSTracker.getInstance().bpsQueue.getFirst().getRight());
                lines.add("   Last timestamp: " + BPSTracker.getInstance().bpsQueue.getLast().getRight());
            }
        }
    }
}
