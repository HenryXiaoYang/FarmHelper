...
import com.jelly.farmhelperv3.util.helper.BaritoneEventListener;
...
public class BaritoneHandler {
    ...
    public static boolean isWalkingToGoalBlock(double nearGoalDistance) {
        if (pathing) {
            ...
            if (goal == null) { pathing = false; return false; }
            if (goal instanceof GoalBlock) {
                GoalBlock goal1 = (GoalBlock) BaritoneAPI.getProvider().getPrimaryBaritone().getPathingBehavior().getGoal();
                distance = mc.player.position().distanceTo(new net.minecraft.world.phys.Vec3(goal1.getGoalPos().getX() + 0.5f, mc.player.getY(), goal1.getGoalPos().getZ() + 0.5));
                ...
                distance = goal.isInGoal(mc.player.blockPosition()) ? 0 : goal.heuristic();
            }
