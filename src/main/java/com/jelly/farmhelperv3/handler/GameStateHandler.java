...
import com.jelly.farmhelperv3.failsafe.impl.JacobFailsafe;
import com.jelly.farmhelperv3.feature.impl.AutoRepellent;
import com.jelly.farmhelperv3.feature.impl.PestsDestroyer;
import com.jelly.farmhelperv3.util.*;
import com.jelly.farmhelperv3.util.helper.Clock;
import com.jelly.farmhelperv3.util.helper.Timer;
...
public class GameStateHandler {
    ...
    public void onTablistUpdate(UpdateTablistEvent event) {
        ...
        if (PlayerUtils.isInventoryEmpty(mc.player) && scoreboardLines.isEmpty() && mc.player.experienceLevel == 0 && mc.level.dimension().equals(net.minecraft.world.level.Level.END)) {
            lastLocation = location;
            location = Location.LIMBO;
            return;
        }
    ...
    public void onTickCheckSpeed() {
        float speed = mc.player.getAbilities().getWalkingSpeed();
        this.speed = (int) (speed * 1_000);
    }

    public void onTickCheckMoving() {
        dx = Math.abs(mc.player.getDeltaMovement().x);
        dy = Math.abs(mc.player.getDeltaMovement().y);
        dz = Math.abs(mc.player.getDeltaMovement().z);

        if (notMoving() && mc.screen == null) {
        ...
        if (MacroHandler.getInstance().getCrop() == FarmHelperConfig.CropEnum.CACTUS) {
            frontWalkable = BlockUtils.canWalkThroughDoor(BlockUtils.WalkDirection.FORWARD) && BlockUtils.canWalkThrough(BlockUtils.getRelativeBlockPos(0, 0, 1, yaw), BlockUtils.WalkDirection.FORWARD) && BlockUtils.getRelativeBlock(0, 0, 2, yaw) != Blocks.CACTUS;
            backWalkable = BlockUtils.canWalkThroughDoor(BlockUtils.WalkDirection.BACKWARD) && BlockUtils.canWalkThrough(BlockUtils.getRelativeBlockPos(0, 0, -1, yaw), BlockUtils.WalkDirection.BACKWARD) && BlockUtils.getRelativeBlock(0, 0, -2, yaw) != Blocks.CACTUS;
        } else {
            frontWalkable = BlockUtils.canWalkThroughDoor(BlockUtils.WalkDirection.FORWARD) && BlockUtils.canWalkThrough(BlockUtils.getRelativeBlockPos(0, 0, 1, yaw), BlockUtils.WalkDirection.FORWARD);
            backWalkable = BlockUtils.canWalkThroughDoor(BlockUtils.WalkDirection.BACKWARD) && BlockUtils.canWalkThrough(BlockUtils.getRelativeBlockPos(0, 0, -1, yaw), BlockUtils.WalkDirection.BACKWARD);
        }

        rightWalkable = BlockUtils.canWalkThroughDoor(BlockUtils.WalkDirection.RIGHT) && BlockUtils.canWalkThrough(BlockUtils.getRelativeBlockPos(1, 0, 0, yaw), BlockUtils.WalkDirection.RIGHT);
        leftWalkable = BlockUtils.canWalkThroughDoor(BlockUtils.WalkDirection.LEFT) && BlockUtils.canWalkThrough(BlockUtils.getRelativeBlockPos(-1, 0, 0, yaw), BlockUtils.WalkDirection.LEFT);
    }
    ...
    public boolean notMoving() {
        if (dx < 0.01 && dz < 0.01 && dyIsRest() && mc.screen == null) {
            return true;
        }
        return !holdingKeybindIsWalkable() && (playerIsInFlowingWater(0) || playerIsInFlowingWater(1)) && mc.player.isInWater();
    }

    public boolean notMovingHorizontally() {
        if (dx < 0.01 && dz < 0.01 && mc.screen == null) {
            return true;
        }
        return !holdingKeybindIsWalkable() && (playerIsInFlowingWater(0) || playerIsInFlowingWater(1)) && mc.player.isInWater();
    }
    ...
    public boolean playerIsInFlowingWater(int y) {
        BlockState state = mc.level.getBlockState(BlockUtils.getRelativeBlockPos(0, y, 0));
    ...
    public boolean holdingKeybindIsWalkable() {
        KeyMapping[] holdingKeybinds = KeyBindUtils.getHoldingKeybinds();
        for (KeyMapping key : holdingKeybinds) {
            if (key != null && key.isDown()) {
                if (key == mc.options.keyUp && !frontWalkable && !FarmHelperConfig.alwaysHoldW) {
                    return false;
                } else if (key == mc.options.keyDown && !backWalkable) {
                    return false;
                } else if (key == mc.options.keyRight && !rightWalkable) {
                    return false;
                } else if (key == mc.options.keyLeft && !leftWalkable) {
                    return false;
                }
            }
        }
