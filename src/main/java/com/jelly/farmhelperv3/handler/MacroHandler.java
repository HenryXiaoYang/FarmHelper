...
import com.jelly.farmhelperv3.failsafe.impl.WorldChangeFailsafe;
...
import com.jelly.farmhelperv3.macro.impl.*;
...
public class MacroHandler {
    ...
    public void enableMacro() {
        ...
        AudioManager.getInstance().setSoundBeforeChange(mc.options.getSoundSourceVolume(SoundSource.MASTER));
...
//            Scheduler.getInstance().resume(); - gets enabled in featuremanager
    ...
    public void enableCurrentMacro() {
        if (currentMacro.isPresent() && !currentMacro.get().isEnabledAndNoFeature() && !startingUp) {
            ...
            KeyBindUtils.holdThese(mc.player.getAbilities().flying ? mc.options.keyShift : null);
            Tasks.schedule(() -> {
                startCurrent.run();
                macroingTimer.resume();
            }, 300, TimeUnit.MILLISECONDS);
    ...
    public void onChatMessageReceived(ClientChatReceivedEvent event) {
            ...
            if (!message.contains(":") && GameStateHandler.getInstance().inGarden()) {
                if (message.equals("Your spawn location has been set!")) {
                    ...
                    for (Rewarp rewarp : FarmHelperConfig.rewarpList) {
                        if (mc.player.position().distanceTo(new net.minecraft.world.phys.Vec3(rewarp.x, rewarp.y, rewarp.z)) < 2) {
        ...
        currentMacro.ifPresent(m -> {
            ...
            m.onChatMessageReceived(event.message.getString());
        });
    }
    ...
    public void onRenderWorldLast(RenderWorldLastEvent event) {
        ...
        if (FarmHelperConfig.highlightRewarp && FarmHelperConfig.rewarpList != null && GameStateHandler.getInstance().inGarden()) {
            for (Rewarp rewarp : FarmHelperConfig.rewarpList) {
                double distance = mc.player.position().distanceTo(new net.minecraft.world.phys.Vec3(rewarp.x, rewarp.y, rewarp.z));
                Color rewarpColor = FarmHelperConfig.rewarpColor.toJavaColor();
                if (distance < 50) {
                    RenderUtils.drawBlockBox(BlockPos.containing(rewarp.x, rewarp.y, rewarp.z), rewarpColor);
                }
            }
        }

        if (FarmHelperConfig.drawSpawnLocation && PlayerUtils.isSpawnLocationSet() && GameStateHandler.getInstance().inGarden()) {
            BlockPos spawnLocation = BlockPos.containing(PlayerUtils.getSpawnLocation());
            double distance = mc.player.position().distanceTo(new net.minecraft.world.phys.Vec3(spawnLocation.getX(), spawnLocation.getY(), spawnLocation.getZ()));
    ...
    private void checkForTeleport() {
        ...
        if (mc.player.blockPosition().distSqr(beforeTeleportationPos.get()) > 2 || PlayerUtils.isStandingOnSpawnPoint()) {
            ...
            if (!mc.player.getAbilities().flying && !mc.player.onGround()) {
                ...
                return;
            } else if (mc.player.getAbilities().flying && !mc.player.onGround()) {
                if (rewarpTeleport) {
                    LogUtils.sendDebug("Player is flying, but is not on ground. Waiting");
                    if (!mc.options.keyShift.isDown()) {
                        KeyBindUtils.holdThese(mc.options.keyShift);
                        Tasks.schedule(KeyBindUtils::stopMovement, (long) (350 + Math.random() * 300), TimeUnit.MILLISECONDS);
                    }
                    return;
                }
            }
    ...
    public void triggerWarpGarden(boolean force, boolean rewarpTeleport, boolean sendErrors) {
        ...
        if (force || GameStateHandler.getInstance().canRewarp() && !beforeTeleportationPos.isPresent()) {
            ...
            setBeforeTeleportationPos(Optional.ofNullable(mc.player.blockPosition()));
