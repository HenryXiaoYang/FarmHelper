...
import com.jelly.farmhelperv3.event.MotionUpdateEvent;
import com.jelly.farmhelperv3.mixin.client.MinecraftAccessor;
...
public class RotationHandler {
    ...
    public Rotation getNeededChange(Rotation target) {
        if (configuration != null && configuration.rotationType() == RotationConfiguration.RotationType.SERVER) {
            ...
            return getNeededChange(new Rotation(mc.player.getYRot(), mc.player.getXRot()), target);
        }
    }
    ...
    public Rotation getNeededChange(Rotation startRot, Rotation endRot) {
        float yawDiff = (float) (wrapDegrees(endRot.getYaw()) - wrapDegrees(startRot.getYaw()));
    ...
    public Rotation getRotation(Vec3 to) {
        return getRotation(mc.player.getEyePosition(mc.getDeltaTracker().getGameTimeDeltaPartialTick(false)), to, false);
    }

    public Rotation getRotation(Vec3 to, boolean randomness) {
        return getRotation(mc.player.getEyePosition(mc.getDeltaTracker().getGameTimeDeltaPartialTick(false)), to, randomness);
    }

    public Rotation getRotation(Entity to) {
        return getRotation(mc.player.getEyePosition(mc.getDeltaTracker().getGameTimeDeltaPartialTick(false)), to.position().add(0, Math.min(((to.getBbHeight() * 0.85) + randomAddition), 1.7), 0), false);
    }
    ...
    public Rotation getRotation(BlockPos pos) {
        if (configuration != null && random.nextGaussian() > 0.8) {
            return getRotation(mc.player.getEyePosition(mc.getDeltaTracker().getGameTimeDeltaPartialTick(false)), new Vec3(pos.getX() + 0.5, pos.getY() + 0.5, pos.getZ() + 0.5), configuration.randomness());
        }
        return getRotation(mc.player.getEyePosition(mc.getDeltaTracker().getGameTimeDeltaPartialTick(false)), new Vec3(pos.getX() + 0.5, pos.getY() + 0.5, pos.getZ() + 0.5), false);
    }

    public Rotation getRotation(BlockPos pos, boolean randomness) {
        return getRotation(mc.player.getEyePosition(mc.getDeltaTracker().getGameTimeDeltaPartialTick(false)), new Vec3(pos.getX() + 0.5, pos.getY() + 0.5, pos.getZ() + 0.5), randomness);
    }
    ...
    public void onRender(RenderWorldLastEvent event) {
        ...
        if (mc.screen != null || dontRotate.isScheduled() && !dontRotate.passed()) {
        ...
        if (System.currentTimeMillis() >= endTime && !configuration.followTarget()) {
            ...
            if (configuration.callback().isPresent()) {
                ...
                mc.player.setYRot(targetRotation.getYaw());
                mc.player.setXRot(targetRotation.getPitch());
                reset();
                return;
            }
        ...
        if (configuration.followTarget() && configuration.target().isPresent()) {
            ...
            if (shouldRotate(targetRotation, 0.1f)) {
                ...
                float scaledFps = 60f / Minecraft.getInstance().getFps();
                needYaw *= scaledFps;
                needPitch *= scaledFps;
                needYaw /= Math.max(distance / 80, 1);
                mc.player.setYRot(mc.player.getYRot() + needYaw);
                if (mc.player.getXRot() + needPitch > 75 && needPitch < 0) {
                    mc.player.setXRot(mc.player.getXRot() + needPitch);
                } else if (mc.player.getXRot() + needPitch < -75 && needPitch > 0) {
                    mc.player.setXRot(mc.player.getXRot() + needPitch);
                } else if (mc.player.getXRot() + needPitch > -75 && mc.player.getXRot() + needPitch < 75) {
                    mc.player.setXRot(mc.player.getXRot() + needPitch);
                }
            }
        } else {
            mc.player.setYRot(interpolate(startRotation.getYaw(), targetRotation.getYaw(), configuration.easeOutBack() ? this::easeOutBack : this::easeOutExpo));
            mc.player.setXRot(interpolate(startRotation.getPitch(), targetRotation.getPitch(), configuration.easeOutBack() ? this::easeOutBack : this::easeOutQuart));
        }
    }
    ...
    public void onUpdatePre(MotionUpdateEvent.Pre event) {
        ...
        clientSidePitch = mc.player.getXRot();
        clientSideYaw = mc.player.getYRot();
        // rotating
        ...
        serverSidePitch = event.pitch;
        serverSideYaw = event.yaw;
        mc.player.setYRot(serverSideYaw);
        mc.player.setXRot(serverSidePitch);
    }
    ...
    private void adjustTargetRotation(boolean serverSide) {
        ...
        startRotation.setPitch(serverSide ? serverSidePitch : mc.player.getXRot());
        startRotation.setYaw(serverSide ? serverSideYaw : mc.player.getYRot());
        startTime = System.currentTimeMillis();
    ...
    public void onUpdatePost(MotionUpdateEvent.Post event) {
        ...
        mc.player.setYRot(clientSideYaw);
        mc.player.setXRot(clientSidePitch);
    }
}
