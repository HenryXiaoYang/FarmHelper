...
public class CobwebFailsafe extends Failsafe {
    ...
    public boolean isTouchingCobwebBlock() {
        for (int x = -1; x <= 1; x++) {
            for (int y = -1; y <= 1; y++) {
                for (int z = -1; z <= 1; z++) {
                    BlockPos pos = new BlockPos(mc.player.blockPosition().offset(x, y, z));
                    double distance = Math.sqrt(mc.player.getEyePosition(1).distanceTo(new Vec3(pos.getX() + 0.5, pos.getY() + 0.5, pos.getZ() + 0.5)));
                    if (mc.level.getBlockState(pos).getBlock().equals(Blocks.COBWEB) && distance <= 1.5) {
                        if (cobwebs.stream().noneMatch(tuple -> tuple.getLeft().equals(pos))) {
                            cobwebs.add(Pair.of(pos, System.currentTimeMillis()));
    ...
    private boolean cobwebsRemoved() {
        cobwebs.removeIf(tuple -> {
            if (System.currentTimeMillis() - tuple.getRight() > 120_000 || !mc.level.getBlockState(tuple.getLeft()).getBlock().equals(Blocks.COBWEB)) {
                LogUtils.sendDebug("[Failsafe] Cobweb removed: " + tuple.getLeft());
                return true;
            }
            return false;
        });
    ...
    private final ArrayList<Pair<BlockPos, Long>> cobwebs = new ArrayList<>();
