...
import com.jelly.farmhelperv3.failsafe.impl.*;
...
//        if (FeatureManager.getInstance().shouldIgnoreFalseCheck()) return;
    ...
    public void possibleDetection(Failsafe failsafe) {
        ...
        if (!sendingFailsafeInfo) {
            ...
            Tasks.schedule(() -> {
                ...
                Tasks.schedule(() -> {
                    if (FarmHelperConfig.autoAltTab && failsafe.shouldAltTab() && !Minecraft.getInstance().isWindowActive()) {
                ...
                if (failsafe.shouldSendNotification())
                    FailsafeUtils.getInstance().sendNotification(ChatFormatting.stripFormatting(tempFailsafe.getType().label), TrayIcon.MessageType.WARNING);
    ...
    public void onTickChooseEmergency(TickEvent.ClientTickEvent event) {
        ...
        chooseEmergencyDelay.reset();
        hadEmergency = true;
        LogUtils.sendDebug("[Failsafe] Emergency chosen: " + ChatFormatting.stripFormatting(triggeredFailsafe.get().getType().name()));
    ...
    public void onRenderOverlay(RenderGameOverlayEvent.Post event) {
        ...
        if (chooseEmergencyDelay.isScheduled()) {
            ...
            textLines.add("§6" + ChatFormatting.stripFormatting(triggeredFailsafe.get().getType().name()).replace("_", " "));
    ...
    public void selectNextItemSlot() {
        int nextSlot = mc.player.getInventory().getSelectedSlot() + 1;
        if (nextSlot > 7) {
            nextSlot = 0;
        }
        if (mc.player.getInventory().getSelectedSlot() != nextSlot) {
