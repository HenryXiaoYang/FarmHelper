...
For Bazaar sell orders, enable **Automation → Auto Sell → Use Bazaar Sell Orders** with the market set to **BZ**. This opt-in opens Bazaar immediately and uses the game's **Same as Best Offer** option for farming/custom items, including farming-sack batches when enabled. Auto Sell makes no external price requests or market-price comparisons. After returning to Garden spawn, it clicks Bazaar's bulk coin-claim button once for all available sell-order proceeds, including manual/older orders and proceeds from partially filled orders. Unfilled quantities stay listed, and buy-order items are not claimed. Initial farming startup is skipped. Failed transactions keep remaining items and back off automatic selling for five minutes; manual Auto Sell bypasses that wait. Menu flows have local fixture coverage; live Hypixel transactions still require verification.
