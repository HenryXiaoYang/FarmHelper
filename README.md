...
For Bazaar sell orders, enable **Automation → Auto Sell → Use Bazaar Sell Orders** with the market set to **BZ**. This opt-in lists farming items and configured custom items at **Same as Best Offer**, including farming-sack batches when enabled. After returning to Garden spawn, it claims completed orders and reprices undercut orders created during the current world session; it skips the initial farming start. Repricing follows the current market without a price-drop floor. Leaving the world or manually interacting with Bazaar orders clears ownership tracking, so older/manual orders are left alone. Failed or blocked sales keep your items, resume farming, and wait five minutes before an automatic retry. Manual Auto Sell bypasses that wait. This mode uses public Bazaar price data and requires no API key. Its menu flow has local fixture coverage; live Hypixel transactions still require verification.
