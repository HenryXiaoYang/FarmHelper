...
- `FarmHelperV3-3.0.2.jar` from `build/libs`.
...
For Bazaar sell orders, enable **Automation → Auto Sell → Use Bazaar Sell Orders** with the market set to **BZ**. Automatic selling starts on a confirmed return to spawn or after inventory continuously stays at or above the configured fullness threshold for at least 3 seconds (defaults: 100%, 3 seconds). Personal Compactor freeing space resets this timer. If automatic pest clearing is enabled, spawn-triggered selling waits until pests are cleared; the full-inventory trigger remains independent. Initial farming startup is skipped, and visitors/composter cannot start extra sales unless inventory is full. Each order-selling pass claims available Bazaar coins before listing and again afterward, using the native bulk coin button for manual/older/partially filled sell orders too. NPC and instant-sale completion also checks for outstanding Bazaar coins. Auto Sell makes no external price requests or market comparisons. A native order-limit rejection triggers one claim-and-refresh recovery; if own buy/sell orders still occupy every slot, remaining items are kept and automatic selling backs off for five minutes. Orders are not cancelled or instant-sold to make room. Manual Auto Sell remains available. Menu flows have local fixture coverage; live server validation follows installation.
